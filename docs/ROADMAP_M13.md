...
- [ ] **Commit 2 — F1+F2 cleanup**: delete dead `gateway/routes/agentmesh.yml` + companion infra-side cleanup; isolated `MASTControllerSliceTest` reproducing the `/api/mast/violations/{recent,unresolved}` 403 under `AUTH_ENFORCED=true` and the matching `MASTController` fix (Spring Security 6 method-vs-class `@PreAuthorize` interaction; Finding F2 in `docs/ACCEPTANCE_M13.3.md`).
