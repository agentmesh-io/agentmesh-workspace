...
- [x] **Commit 7 — v1.1.0 tag + GitHub release**: `docs/RELEASE_NOTES_v1.1.md` (code name **"Adoption Train"**) ships TL;DR + 7-commit acceptance table + numbers-vs-v1.0 trend + dual upgrade paths (Compose + Helm). Annotated `v1.1.0` tag pushed to **both** `agentmesh-io/agentmesh-workspace` (this commit) and `agentmesh-io/agentmesh` (placed at `e166b4f` — the F2 fix). GitHub releases created in both repos via `gh release create`. **Sprint M13.3 closed: 7/7 commits, all acceptance matrices green.** Evidence: [`docs/RELEASE_NOTES_v1.1.md`](RELEASE_NOTES_v1.1.md), [`docs/ACCEPTANCE_M13.3.md`](ACCEPTANCE_M13.3.md) §"Commit 7" + §"Sprint M13.3 — final scoreboard".
- **Acceptance:** ✅ Helm chart `lint` + `template` clean for all 3 stages (dev=6 / staging=10 / prod=9 resources, 0 unsubstituted-template sentinels); `make demo` finishes without manual intervention (~3 min cold, ~30 s warm) with `scripts/demo-smoke.sh` returning **PASS=9 FAIL=0**; UAT **21/21**. Evidence: [`docs/ACCEPTANCE_M13.3.md`](ACCEPTANCE_M13.3.md). Sprint M13.3 closed 2026-04-30 with `v1.1.0` tag.
